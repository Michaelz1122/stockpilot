...
export default function SaleDetail() {
  ...
  if (!inv.data) {
    return (
      <Screen>
        <Header title={t('invoices.sales')} showBack />
        <Text className="text-muted-foreground">{t('common.loading')}</Text>
      </Screen>
    );
  }
  const i = inv.data;
  const customer = (customers.data ?? []).find((c: any) => c.id === i.customer_id);
  ...
  const onShare = () =>
    shareInvoiceAsText({
      ...
      partyLabel: t('invoices.customer'),
      partyName: customer?.name ?? t('common.walkIn'),
      thanksLine: t('invoice.thanksLine'),
    });

  const onDelete = () =>
    Alert.alert(t('invoices.deleteTitle'), t('invoices.deleteSalePrompt'), [
      { text: t('common.cancel'), style: 'cancel' },
      {
        text: t('common.delete'),
        style: 'destructive',
        onPress: async () => {
          await SalesRepo.remove(i.id);
          router.back();
        },
      },
    ]);
  ...
  return (
    <Screen padded={false}>
      ...
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 120 }}>
        {/* Hero — store + invoice number */}
        <View className="rounded-3xl bg-gradient-to-br from-brand-600 to-brand-800 p-5"
          ...
          <View className="flex-row items-start justify-between">
            ...
            <View
              className={`rounded-full px-3 py-1 ${
                paidFull ? 'bg-emerald-500' : 'bg-amber-500'
              }`}
            >
                  ...
                  <View className="flex-1">
                    ...
                    <View className="mt-1 flex-row gap-3">
                      <Text className="text-xs text-muted-foreground">
                        {formatNumber(item.quantity)} × {formatMoney(item.unit_price, store?.currency)}
                      </Text>
                      {Number(item.discount) > 0 && (
                        <Text className="text-xs text-amber-600">
                          -{formatMoney(item.discount, store?.currency)}
                        </Text>
                      )}
                    </View>
                  </View>
        ...
        <Card className="mt-4">
          <View className="flex-row justify-between">
            ...
            <Text className="text-card-foreground">
              {formatMoney(i.subtotal, store?.currency)}
            </Text>
          </View>
          {Number(i.discount) > 0 && (
          ...
          <View className="mt-2 flex-row justify-between">
            ...
            <Text className="text-card-foreground">
              {formatMoney(i.paid, store?.currency)}
            </Text>
          </View>
          {remaining > 0 && (
            <View className="mt-1 flex-row justify-between">
